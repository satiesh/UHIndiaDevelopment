...
import { StockTradeCommentComponent } from '@app/components/admin/optionstrade-editor/stocktrade-comment.component';
import { DatePipe } from '@angular/common';
import { AnonymousSubject } from 'rxjs/internal/Subject';


@Component({
  selector: 'app-new-daily-trade',
  templateUrl: './new-daily-trade.component.html',
  styleUrls: ['./new-daily-trade.component.scss'],
  providers: [DatePipe]
})
export class NewDailyTradeComponent implements OnInit {
 ...
 // @Input() allTradeData: optionstrade[];
  ...
  today: any; todayDate: any; EveryDayTrade = [];
  allDate = [];
  todaytradeArray: optionstrade[];
  ...
  createTodayDate() {
   const dattdy = new Date()
   this.todayDate = this.datepipe.transform(dattdy, 'yyyy-MM-dd');
   console.log('Today Date' , this.todayDate);
  }

  checkTodayDate(dateData: any) {
    var dateWrapper = new Date(dateData);
    if (isNaN(dateWrapper.getDate())) {
      var dat1 =  Utilities.transformSecondsToDate(dateData);
      this.allDate.push({ HistoryofDate: this.datepipe.transform(dat1, 'yyyy-MM-dd')});
    }
    else {
      return dateData;
    }
  ...
  private onDataLoadSuccessful(optionstrade: optionstrade[]) {
    ...
    optionstrade.forEach(item => {
     this.checkTodayDate(item.createdon);
    })
    let arr3 = optionstrade.map((item, i) => Object.assign({}, item, this.allDate[i]));
    this.todaytradeArray = arr3.filter((data) => {
      return data.HistoryofDate == this.todayDate;
    })
    this.dataSource.data =  this.todaytradeArray;
    this.EveryDayTrade = this.todaytradeArray
    console.log('New Array' , this.todaytradeArray[0])
    console.log('New Array 1...' , this.todaytradeArray[0].stopLoss)
  }
