...
import { usersubscription, subscriptions, userroles, userprofile, userdisclaimer, usermessaging, userpayment } from '../models';
...
export class Utilities {
  ...
  public static checkDate(dateData: any) {
    var dateWrapper = new Date(dateData);
    if (isNaN(dateWrapper.getDate())) {
      return this.transformSecondsToDate(dateData);
    }
    else {
      return dateData;
    }
  ...
  public static transformSecondsToDate(secondsValu: any): Date {
    if (secondsValu) {
  ...
  public static newUserDTO(currentUserId?: string): User {
    let nUser: User = new User();
    nUser.userprofile = new userprofile();
    nUser.userdisclaimer = new userdisclaimer();
    nUser.usermessaging = new usermessaging();
    nUser.userothervalues = new userothervalues();
    nUser.userpayment = [];
    nUser.userquestions = [];
    nUser.userroles = new userroles();
    let nsubscriptions: subscriptions[] = [];
    nUser.usersubscription = new usersubscription();
    nUser.usersubscription.subscriptions = nsubscriptions;
    nUser.userquestions = [];
    return nUser;

  }

  public static userDTO(args: User, currentUserId?: string): User {
    let nUser: User = new User(args.uid, args.email, args.photoURL,
      args.emailVerified, args.isNewUser, [], args.memberId ? args.memberId:'',args.disabled? args.disabled:false);
    nUser.displayName = args.displayName;
    if (args.userdisclaimer) {
      nUser.userdisclaimer = args.userdisclaimer;
    }
    if (args.userprofile) {
      let nuserprofile: userprofile = new userprofile(args.userprofile.mobileNumber, args.userprofile.firstName, args.userprofile.lastName,
        args.userprofile.address, args.userprofile.address1, args.userprofile.city, args.userprofile.state, args.userprofile.postalZip, args.userprofile.country,
        args.userprofile.createdOn, args.userprofile.createdBy
      )
      nUser.userprofile = nuserprofile;
    }
    if (args.hasOwnProperty('userroles')) {
      let nuserroles: userroles = new userroles(args.userroles.roleName, args.userroles.createdOn, args.userroles.createdBy);
      nUser.userroles = nuserroles;
    }
    else {
      var date = new Date();
      nUser.userroles = new userroles('', date, currentUserId);
    }
    if (args.hasOwnProperty('userothervalues')) {
      let nuserothervalues: userothervalues = new userothervalues(
        args.userothervalues.investorLevel,
        args.userothervalues.investmentType,
        args.userothervalues.tradingTools,
        args.userothervalues.investingobjective,
        args.userothervalues.bestCommunicationMethod,
        args.userothervalues.bestTimeToCall,
        args.userothervalues.professionalField,
        args.userothervalues.discordID,
        args.userothervalues.currentPortfolioSize,
        args.userothervalues.volunteer,
        args.userothervalues.ipaddress,
        args.userothervalues.createdOn,
        args.userothervalues.createdBy);
      nUser.userothervalues = nuserothervalues;
    }
    else {
      var date = new Date();
      nUser.userothervalues = new userothervalues('', '', '','','','','','','','','', date, currentUserId);
    }
    if (args.userdisclaimer) {
      nUser.userdisclaimer = args.userdisclaimer;
    }
    nUser.userpayment = [];
    for (var i = 0; i < args.userpayment.length; i++) {
      nUser.userpayment.push(args.userpayment[i]);
    }
    nUser.usersubscription = new usersubscription();
    nUser.usersubscription.subscriptions = [];
    if (args.usersubscription) {
      for (var i = 0; i < args.usersubscription.subscriptions.length; i++) {
        let osubscriptions: subscriptions = new subscriptions(
          args.usersubscription.subscriptions[i].subscriptionId,
          args.usersubscription.subscriptions[i].isActive,
          args.usersubscription.subscriptions[i].renewalOn,
          args.usersubscription.subscriptions[i].createdOn,
          args.usersubscription.subscriptions[i].createdBy
        )
        nUser.usersubscription.subscriptions.push(osubscriptions);
      }
    }
    return nUser;
  }

  public static getCurrentDateUTC(): number {
    var date = new Date();
  ...
  public static getRenewalDate(Renewaltype: string): Date {
    var date = new Date();
    var renewalDate: Date;
    switch (Renewaltype.toLowerCase()) {
      case "yearly":
        renewalDate = new Date(date.setFullYear(date.getFullYear() + 1));
        break;
      case "monthly":
        renewalDate = new Date(date.setMonth(date.getMonth() + 1));
        break;
      case "free":
        renewalDate = new Date(date.setFullYear(date.getFullYear() + 10));
        break;
      default:
    }
    return renewalDate;
  ...
  public static getRandomInt(length: number): number {
