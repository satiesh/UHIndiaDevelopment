...
import { EditDailyTradeDialogComponent } from '../dailytrade-dialog/dailytrade-dialog.component';

@Component({
  selector: 'app-dailytrade-list',
  templateUrl: './dailytrade-list.component.html',
  styleUrls: ['./dailytrade-list.component.scss'],
  animations: [fadeInOut]
})

export class DailyTradeListComponent implements OnInit, AfterViewInit {
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;

  displayedColumns = ['symbol', 'support', 'resistancetarget', 'stoploss', 'createdon'];
  ...
  constructor(
    ...
    if (this.canManageStockOftheDay) {
  ...
  ngOnInit() {
    ...
    this.selectDailyTicker$ = this.store$.select(DailyTradeStoreSelectors.selectDailyTrades);
    this.loadData();
  }
  ...
  editDailyTicker(ticker?: dailyticker) {
    this.sourceDailyTicker = ticker;
    const dialogRef = this.dialog.open(EditDailyTradeDialogComponent,
      {
